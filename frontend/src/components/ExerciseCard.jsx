        ...
        <div className={`bg-brand-light-gray p-4 rounded-xl shadow-sm border border-brand-border flex flex-col gap-3 group relative ${isReadOnly ? 'opacity-80' : ''}`}>
            ...
            <div className="flex gap-4 items-start">
                <div className="flex flex-col gap-1 w-1/3">
                    ...
                    <select
                        disabled={isReadOnly}
                        className="bg-brand-gray rounded-lg p-2 font-medium text-gray-300 outline-none focus:ring-2 focus:ring-brand-lime transition-all w-full text-sm appearance-none disabled:opacity-50"
                        value={currentType}
                ...
                <div className="flex flex-col gap-1 w-1/4">
                    ...
                    <input
                        disabled={isReadOnly}
                        type="number"
                        className="bg-brand-gray rounded-lg p-2 font-medium text-gray-300 outline-none focus:ring-2 focus:ring-brand-lime transition-all w-full disabled:opacity-50"
                        placeholder="0"
                        value={exercise.sets}
                        ...
                        <div className="flex flex-wrap gap-2">
                            {Array.from({ length: parseInt(exercise.sets) || 0 }).map((_, i) => (
                                <input
                                    ...
                                    disabled={isReadOnly}
                                    type="number"
                                    className="bg-brand-gray rounded-lg p-2 font-medium text-gray-300 outline-none focus:ring-2 focus:ring-brand-lime transition-all w-16 text-center disabled:opacity-50"
                                    placeholder={`Rep ${i + 1}`}
                                    value={getRepValue(i)}
